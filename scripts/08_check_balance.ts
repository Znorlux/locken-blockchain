...
Registering user with
 Address:${userAddress.toLowerCase()}`;
    ...
    try {
      ...
      if (tokenId === 0n) {
        console.log(
          "📋 Token not registered in EncryptedERC yet - no encrypted balance"
        );
      ...
      console.log(
        "📋 Token not registered in EncryptedERC - no encrypted balance"
      );
      console.log("💡 Make a deposit first: npm run deposit");
      return;
    }
    ...
    if (isEGCTEmpty) {
      console.log(
        "📋 EGCT is empty (all zeros) - no main encrypted balance found"
      );
      ...
      if (amountPCTs.length > 0) {
        ...
        for (let i = 0; i < amountPCTs.length; i++) {
          ...
          try {
            if (amountPCT.pct.some((e: bigint) => e !== 0n)) {
              ...
              console.log(
                `  - Amount PCT ${i}: ${decryptedAmount[0].toString()} (index: ${amountPCT.index})`
              );
        ...
        console.log(
          "📋 No Amount PCTs found - user has no transaction history"
        );
      }
    ...
    const encryptedSystemDecimals = 2;
    console.log(
      `🔒 EGCT Balance: ${ethers.formatUnits(egctBalance, encryptedSystemDecimals)} encrypted units`
    );
    ...
    console.log("\n📊 Balance Summary:");
    console.log(
      `   EGCT Balance (main): ${ethers.formatUnits(egctBalance, encryptedSystemDecimals)} encrypted units`
    );
    if (totalFromPCTs > 0n) {
      console.log(
        `   Total from PCTs: ${ethers.formatUnits(totalFromPCTs, encryptedSystemDecimals)} encrypted units`
      );
    }
    console.log(
      `   Public ${tokenSymbol} Balance: ${ethers.formatUnits(tokenBalance, tokenDecimals)} ${tokenSymbol}`
    );

    // Show equivalent values
    console.log("\n💡 Balance Explanation:");
    console.log(
      `   • EGCT Balance: Your main encrypted balance stored on-chain`
    );
    console.log(`   • Amount PCTs: Transaction history for auditing purposes`);
    console.log(
      `   • Public Balance: Your regular ${tokenSymbol} tokens (not encrypted)`
    );
    console.log(
      `   • Encrypted System: Uses ${encryptedSystemDecimals} decimal places internally`
    );
    ...
    if (egctBalance !== totalFromPCTs && totalFromPCTs > 0n) {
      ...
      console.log(
        "   This might indicate an issue with the encryption/decryption process"
      );
    ...
    if (
      ...
      JSON.parse(fs.readFileSync(keysPath, "utf8")).userAddress !== userAddress
    ) {
      const keysData = {
        userAddress,
        signature,
    ...
    if (error instanceof Error) {
      if (error.message.includes("User not registered")) {
        console.error(
          "💡 Hint: Register your user first with: npm run register:user"
        );
